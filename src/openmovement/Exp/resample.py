...
def resample_fixed(sample_values, use_time=None, in_frequency=None, out_frequency=None, lp_filter=True,
    ...
    * 0 or other value - output includes an initial time column that starts at the specified time (seconds).
    ...
    if in_frequency is None or hasattr(in_frequency, "__len__"):
        if hasattr(sample_values, 'dtype') and hasattr(sample_values.dtype,
                                                       'metadata') and sample_values.dtype.metadata != None and 'fs' in sample_values.dtype.metadata:
    ...
    # Display plan
    print(
        'RESAMPLE: gcd %d; %d axes; %d samples; in %d Hz; upsample 1:%d; intermediate %d Hz; low-pass %d Hz; downsample %d:1; out %d Hz;' % (
        divisor, multi_axis, sample_values.shape[0], in_frequency, up_sample, intermediateFrequency, lowPass,
        down_sample, out_frequency))
    print(data)
    ...
    if up_sample > 1 and len(data) > 0:
        ...
        print('RESAMPLE: Upsample: %d axes - %d samples at %d Hz --> 1:%d --> %d samples at %d Hz' % (
        multi_axis, len(data), in_frequency, up_sample, intermediateCount, intermediateFrequency))
        # ...
        ...
        if multi_axis > 0:
            ...
            for axis in range(0, multi_axis):
                up_sampled_data[:, axis] = scipy.interpolate.interp1d(inIndex, data[:, axis], kind=interpolation_mode)(
                    outIndex)
    ...
    # Low-pass filter
    if lowPass > 0:
        print('RESAMPLE: Low-pass filter at %d Hz (intermediate at %d Hz, output at %d Hz)' % (
        lowPass, intermediateFrequency, out_frequency))
        data = filter(data, sample_freq=intermediateFrequency, low_freq=None,
                      high_freq=lowPass)  # order=4, type='butter', method='ba'
        print(data)
    else:
    ...
    if down_sample > 1 and len(data) > 0:
        ...
        print('RESAMPLE: Downsample: %d axes - %d samples at %d Hz --> %d:1 --> %d samples at %d Hz' % (
        multi_axis, len(data), intermediateFrequency, down_sample, outputCount, out_frequency))
        if multi_axis > 0:
...
def resample_variable(samples, frequency=None, interpolation_mode='nearest', start_time=None, end_time=None,
                      maximum_missing=7 * 24 * 60 * 60):
    """
    ...
    if frequency is None or hasattr(frequency, "__len__"):
        if hasattr(samples, 'dtype') and hasattr(samples.dtype,
                                                 'metadata') and samples.dtype.metadata != None and 'fs' in samples.dtype.metadata:
    ...
    print("RESAMPLE: %d incoming samples from times %d-%d to %d outgoing samples at %d Hz" % (
    len(samples), start_time, end_time, sample_count, frequency))
    ...
    for axis in range(1, num_axes + 1):
        resampled_data[:, axis] = scipy.interpolate.interp1d(in_timestamps, samples[:, axis], kind=interpolation_mode)(
            out_timestamps)
